...
export class SpaceMinesweeper extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void 
    {
        ...
        this.stars.children.forEach((star: gfx.Transform2) => {
            ...
            const mouseInverse = gfx.Vector2.inverse(this.mousePosition);
            const starMovement = gfx.Vector2.subtract(mouseInverse, this.ship.position);

            starMovement.normalize();
            starMovement.multiplyScalar(shipSpeed);
            star.position.add(starMovement);
        ...
        this.mines.children.forEach((mine: gfx.Transform2) => {
            ...
            const mouseInverse = gfx.Vector2.inverse(this.mousePosition);
            const mineMovement = gfx.Vector2.subtract(mouseInverse, this.ship.position);

            mineMovement.normalize();
            mineMovement.multiplyScalar(shipSpeed);
            mine.rotation += mineRotationSpeed;
            mine.position.add(mineMovement);
    ...
    onMouseDown(event: MouseEvent): void 
    {
        ...
        laserInstance.position.set(this.ship.position.x, this.ship.position.y);
    ...
    private checkForMineCollisions(): void
    {
        for(let i=0; i < this.mines.children.length; i++)
        {
            for(let j=i+1; j < this.mines.children.length; j++)
            {
                ...
                if (mine1.intersects(mine2) == true){
                    if (!mine1.isExploding())
                        mine1.explode();
                    if (!mine2.isExploding())
                        mine2.explode();
                }
    ...
    private checkForLaserCollisions(): void
    {
        this.lasers.children.forEach((laser: gfx.Transform2)=>{
            this.mines.children.forEach((mineElem: gfx.Transform2)=>{
                ...
                if (laser.intersects(mine, gfx.IntersectionMode2.AXIS_ALIGNED_BOUNDING_BOX) == true){
                    if (!mine.isExploding()){
                        mine.explode();
                        laser.remove();
                    }
